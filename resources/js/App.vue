<template>
    <div class="bg-gradient-hero h-[680px] absolute top-0 left-0 w-full -z-10">
        <img src="./assets/ellipse.png" class="w-full absolute top-0 h-[225px] md:h-[261px]" alt="bg" />
    </div>
    <div
        class="container mx-auto px-4 md:px-0 pt-3 md:pt-6 text-blue-darker"
        :class="isMenuOpen ? 'bg-blue-lighter flex flex-col w-screen h-screen fixed z-30 left-0 top-0' : ''"
    >
        <div class="px-6 pt-5 pb-4 flex justify-between items-center container mx-auto bg-blue-lighter/60 rounded-full">
            <div class="flex">
                <router-link to="/" class="font-display text-xl md:text-2xl"> Floating arboretum </router-link>
                <nav class="hidden lg:flex ml-8 space-x-6">
                    ...
                    <router-link to="/trees" class="text-lg group inline-block">
                        {{ $t("Discover stories") }}
                        <span
                            class="block group-hover:opacity-40 transition h-px bg-blue-darker -mt-px"
                            :class="$route.path === '/trees' ? 'opacity-40' : 'opacity-0'"
                        ></span>
                    </router-link>
            ...
            <div>
                ...
                <button @click="toggleMenu" class="md:hidden ml-4">
                    <svg class="w-6 h-auto fill-none stroke-blue-darker" viewBox="0 0 24 17" v-if="!isMenuOpen">
                        <path d="M0.5 0.5H24" />
                        <path d="M0.5 8.5H24" />
                        <path d="M0.5 16.5H24" />
                    </svg>
                    <svg v-else class="w-6 h-auto fill-none stroke-blue-darker" viewBox="0 0 24 19">
...
<style scoped>
.bg-gradient-hero {
    background: linear-gradient(180deg, #90b5dc 0%, #90b5dc 52.78%, #608db5 53.5%, #4b759a 100%);
}
</style>
